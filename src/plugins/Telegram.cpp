...
namespace micasa {
	...
	Telegram::Telegram( const unsigned int id_, const Plugin::Type type_, const std::string reference_, const std::shared_ptr<Plugin> parent_ ) :
	...
	json Telegram::getSettingsJson() const {
		...
		for ( auto &&setting : Telegram::getEmptySettingsJson( true ) ) {
			result.push_back( setting );
		}
		return result;
	};

	json Telegram::getEmptySettingsJson( bool advanced_ ) {
		...
		try {
			...
			if ( result.is_array() ) {
				for ( auto updatesIt = result.begin(); updatesIt != result.end(); updatesIt++ ) {
					...
					if ( device != nullptr ) {
						device->updateValue( Device::UpdateSource::PLUGIN, jsonGet<std::string>( message, "text" ) );
					}
				}
			}
