...
srp6_server_genpub_ex(SRP * srp, cstr ** result, BigInteger k)
{
  ...
  /* oldckhash: B */
  SHAUpdate(&SERVER_CTXP(srp)->oldckhash, bstr->data, bstr->length);
		...
		const unsigned char * pubkey, int pubkeylen)
{
  ...
  SHAUpdate(&SERVER_CTXP(srp)->ckhash, s->data, s->length);
